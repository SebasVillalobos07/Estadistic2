...
La conclusión que se puede llegar a raíz del análisis de estas variables es que ambas bases de datos si pueden ser clusterizadas de manera óptima, mas no puede realizar el análisis factorial de manera confirmatoria al aumentar una variable nueva.

G) Anexos

Aquí se presentan los procesos de análisis tanto en regresión clusterización y factoriales
...
```{r}
library(polycor)
corMatrix=polycor::hetcor(allData)$correlations
```
