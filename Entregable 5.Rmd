---
title: "Entregable 5"
author: "Armando Sebastian Villalobos Blass - 20180820"
date: "18/6/2022"
...
B) Objetivo del estudio del caso
...
c) Explicación de la variable dependiente y sustentación con literatura las variables independientes.
...
La metodología que se utilizó para el caso de estudio es del análisis de regresión, el proceso de clusterización de dos bases de datos con elementos comunes y el análisis factorial de la clusterización.
...
F) Proceso de clusterización de bases de datos.
...
1. Vemos los nombres y ajustamos para una unión correcta

```{r}
list(names(data1), names(data2))
```

2. Se realiza el MERGE más sencillo
...
3. Observamos la estructura (descripción estadísitica)

```{r}
summary(allData)
```

4. Observamos la distribución (posible transformación)
...
6. Ahora, hacemos el proceso de clusterización
...
8. Proponemos la cantidad de clusters
...
9. Resultado preliminar del cluster

```{r}
aggregate(.~ pam, data=dataClus,mean)
```

10. Recodificación del cluster
...
11. Proposición de cantidad de clusters:
...
Ahora damos paso a la estrategia jerárquica que se divide en: 
...
1. Resultado preliminar del cluster

```{r}
aggregate(.~ agnes, data=dataClus,mean)
```

2. Recodificación del cluster
...
1. Utilizamos la estrategia de escalamiento multidimensional
...
6. Ahora calculamos el epsilon mediante dbscan
...
8. Ahora lo colocamos en otra columna
...
*Observamos que la mejor opción es el resultado DIANA

12. Ahora usamos el método jerárquico divisivo
...
13. Ahora podemos usar la teoría de conjuntos para ver qué casos están mal clusterizados en todas las técnicas.
...
G) Análisis de cluster - gráfico final
...
H) Análisis de factorial

1. Ahora pasamos a realizar el análisis factorial (preparatorio)
...
4. Observamos si los datos nos permiten factorizar
...
7.Determinamos en cuantos factores o variables latentes podríamos redimensionar la data

```{r}
fa.parallel(theData,fm = 'ML', fa = 'fa',correct = T)
```

8. Redimensionamos el número de factores
...
9. Resultado mejorado

```{r}
print(resfa$loadings,cutoff = 0.5)
```

I) Análisis de factorial - Resultado visual

```{r}
fa.diagram(resfa)
```

1. ¿Qué variables aportaron más a los factores?

```{r}
sort(resfa$communality)
```

2. ¿Qué variables contribuyen a más de un factor?

```{r}
sort(resfa$complexity)
```

3. ¿Qué nombre le darías?

```{r}
as.data.frame(resfa$scores)%>%head()
```

4. Le daría nombre PBI
...
5. Ahora pasamos a realizar el análisis factorial (confirmatorio)
...
7. Si cada indicador tiene una buena conexión con su latente (ver p valor)

```{r}
allParamCFA[allParamCFA$op=="=~",]
```

8. El ChiSquare es NO significativo? (p_value debe ser mayor a 0.05 para que sea bueno)

```{r}
allFitCFA[c("chisq", "df", "pvalue")]
```

9. El Índice Tucker Lewi es mayor a 0.9?

```{r}
allFitCFA$tli
```

10. La Raíz del error cuadrático medio de aproximación es menor a 0.05?

```{r}
allFitCFA[c('rmsea.ci.lower','rmsea' ,'rmsea.ci.upper')]
```

11. Ya sabemos que no hay buen augurio.
...
13. Veamos ambos scores calculados
